...
@Component({
  selector: 'app-applicationform-sec',
  templateUrl: './applicationform-sec.component.html',
  styleUrls: ['./applicationform-sec.component.css']
})
export class ApplicationformSecComponent implements OnInit {
  ...
  .subscribe(
      data =>{
        ...
        if (data.hasOwnProperty("result")){
          ...
          this.proposalInput.policy.annualPremium = this.proposalInput.policy.basePremium;
          this.proposalInput.insured.personalInfo.height=this.proposalInput.insured.personalInfo.height;
      ...
      if(insuredType == 'IN'){
        ...
        switch(attribute){
          case 'AGEPROOF':  
                          ...
                          break;
          case 'IDPROOF':   
                            obj.displayName =  "Secondary Insured - "+proofType+"(Identity Proof)"; 
                            obj.fileType = "CDF";
                            obj.docId = "2023010118";
                            obj.fileName = "";
                            if (!this.itemExists(this.filesToUpload, obj.docId))
                            this.filesToUpload.push(obj);	
                            else this.filesToUpload[11].displayName = obj.displayName;
                          break;
          case 'INCOMEPROOF':  
...
download1()
{
 ...
 .then((canvas) => {
  ...
  for (var i = 0; i <= quotes.clientHeight/980; i++) {
      ...
      ctx.clearRect(0,0,1390,980);
      ctx.fillStyle="#ffff";
      ctx.fillRect(0,0,1390,980);
      // details on this usage of this function: 
      // https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial/Using_images#Slicing
      ctx.drawImage(srcImg,sX,sY,sWidth,sHeight,dX,dY,dWidth,dHeight);

      // document.body.appendChild(canvas);
      var canvasDataURL = canvas1.toDataURL("image/jpeg", 0.8);
  ...
  let tdysec=tdydate.getSeconds();
   //let downloadpdf="Application_Form_:"+tdydate1+":"+tdymonth+":"+tdyyear+"_Time_"+tdyhour+":"+tdyminute+":"+tdysec;
   let downloadpdf=appNo+"_000000000_111199_"+tdydate1+tdymonth+tdyyear;
  ...
  var pdf = doc.output('blob');
   ...
   //alert(pdf);
  uploadUrlPdf(URL, requestHttp, pdf,"111199", dataapp.proposalData)
            .subscribe(data =>{
            if (!data.hasOwnProperty("error"))
             {
              ...
              dataapp.proposalData.jointLifeappUrl=appDataillurl;
   ...
   /*  html2canvas(document.getElementById('dbs')).then(function(canvas) {
          var imgData = canvas.toDataURL("image/jpeg",0.4);
        //var imgData = canvas.toDataURL("image/jpeg")
           ...
           var options = {
            ...
            doc.addHTML(document.getElementById('dbs'), options, function(){
              doc.save(downloadpdf+".pdf");
              var pdf = doc.output('blob');
