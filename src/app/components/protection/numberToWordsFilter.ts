    ...
    a = [
      ...
      transform(value: any, args?: any): any {
        if (value) {
          ...
          if (num) {
            if ((num = num.toString()).length > 10)  { return ''; }
            const n = ('00000000000' + num).substr(-11).match(/^(\d{2})(\d{2})(\d{2})(\d{2})(\d{1})(\d{2})$/);
            if (!n) {return ''; }
            let str = '';
            str += (Number(n[1]) !== 0) ? (this.a[Number(n[1])] || this.b[n[1][0]] + ' ' + this.a[n[1][1]]) + 'Hundred crore ' : '';
            ...
            (this.a[Number(n[6])] || this.b[n[6][0]] + ' ' +
            this.a[n[5][1]])  : '';
            return str;
          } else {
            return '';
